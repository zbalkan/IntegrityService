...
namespace IntegrityService.Jobs
{
    internal class FileSystemDiscoveryJob
    {
        ...
        internal void Start()
        {
            ...
            Debug.WriteLine("Number of all files in the device: {0}\n" +
                "Number of files to be monitored: {1}\n" +
                "Filtered out: {2} (%{3})",
                filesCount, filteredAfterLastScanCount, diff, percentage);
            _logger.LogInformation("Number of all files in the device: {files:l}\n" +
        ...
        private ConcurrentBag<string> RunNtfsDiscovery(out Stopwatch sw)
        {
            ...
            Debug.WriteLine("Number of all files in the device: {0}", files.Count.ToString("N0"));
            return files;
        }
